  ...
  return (
    <div>
      ...
      <section 
        ...
        style={{ backgroundImage: `url("https://images.unsplash.com/photo-1584515933487-779824d29309?auto=format&fit=crop&w=1200&q=60")` }}
      >
        <div className="bg-black/50 w-full h-full py-28 px-6 md:px-20">
          <h1 className="text-5xl md:text-6xl font-bold mb-4">
            Donate <span className="text-red-500">Blood</span>, Save <span className="text-red-500">Lives</span> ❤️
          </h1>
      ...
      <div className="max-w-6xl mx-auto mt-16 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-10">
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
            ...
            <p className="text-3xl font-bold text-purple-600">{totalRequests}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-14">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold text-center mb-3 text-gray-600">Request Fulfillment Status</h2>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold text-center mb-3 text-gray-600">Blood Group Demand</h2>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={barData}>
                <XAxis dataKey="bloodGroup" /><YAxis allowDecimals={false} />
      ...
      <div className="max-w-6xl mx-auto px-4 mt-16 mb-20 bg-white rounded-lg shadow p-6">
        <h2 className="text-2xl font-semibold mb-4 text-red-600">📝 Latest Requests</h2>
        {receivers.length === 0 && <p className="text-gray-500">No requests yet...</p>}

        {receivers.slice(-5).reverse().map(r => (
          <div key={r._id} className="border-b py-3">
            <strong>{r.name}</strong> needs
            <span className="text-red-600 font-semibold"> {r.bloodGroupNeeded} </span>
            in {r.city} — ({r.status})
          </div>
        ))}
      </div>
